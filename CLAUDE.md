...
- **Ports en `shared-messaging`**: `IMessagePublisher`, `IMessageConsumer`, `INotificationSubscriber` e `ILogger` definidos en el package compartido; adapters Kafka en cada servicio bajo `infrastructure/messaging/kafka/`.
- **Observabilidad**: structured logging via `ILogger` port + `PinoLoggerAdapter` (pino). Distributed tracing via OpenTelemetry SDK con W3C Trace Context (Kafka headers, Redis `_trace` envelope, HTTP headers). `correlation_id` se propaga en toda la cadena (Kafka → Redis → SSE → BFF). **Regla crítica**: el side-effect import de `otel-sdk.ts` debe ser la primera línea de `main.ts`, antes de cualquier import de NestJS — si no, las instrumentaciones de kafkajs/ioredis no se registran y las trazas se pierden silenciosamente. `LOG_LEVEL` env var controla el nivel de log por servicio (default: `info`).
