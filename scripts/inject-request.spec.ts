...
// ─────────────────────────────────────────────────────────────────────────────
// Suite: KAFKA_BROKERS env var sourcing (LocalStack MSK infra)
// Contract: scripts/msk-env.sh (generated) or direct env var.
// The injection script reads KAFKA_BROKERS from process.env with fallback
// to localhost:9092 (confluentinc Kafka).
// ─────────────────────────────────────────────────────────────────────────────

describe('KAFKA_BROKERS env var sourcing', () => {
  // Test 14 — KAFKA_BROKERS se lee de process.env
  it('uses KAFKA_BROKERS from environment when set', () => {
    const original = process.env['KAFKA_BROKERS'];
    try {
      process.env['KAFKA_BROKERS'] = 'localhost:4511';
      const kafkaBrokers = (process.env['KAFKA_BROKERS'] ?? 'localhost:9092').split(',');
      assert.deepEqual(kafkaBrokers, ['localhost:4511']);
    } finally {
  ...
  // Test 15 — Falls back to localhost:9092 when KAFKA_BROKERS is not set
  it('falls back to localhost:9092 when KAFKA_BROKERS is not set', () => {
    const original = process.env['KAFKA_BROKERS'];
    try {
      delete process.env['KAFKA_BROKERS'];
      const kafkaBrokers = (process.env['KAFKA_BROKERS'] ?? 'localhost:9092').split(',');
      assert.deepEqual(kafkaBrokers, ['localhost:9092']);
    } finally {
      if (original !== undefined) {
        process.env['KAFKA_BROKERS'] = original;
      }
  ...
  // Test 16 — soporta múltiples brokers separados por coma
  it('supports multiple brokers separated by comma', () => {
    const original = process.env['KAFKA_BROKERS'];
    try {
      process.env['KAFKA_BROKERS'] = 'localhost:4511,localhost:4512';
      const kafkaBrokers = (process.env['KAFKA_BROKERS'] ?? 'localhost:9092').split(',');
      assert.deepEqual(kafkaBrokers, ['localhost:4511', 'localhost:4512']);
    } finally {
      if (original === undefined) {
        delete process.env['KAFKA_BROKERS'];
      } else {
  ...
  // Test 17 — trims whitespace from broker addresses
  it('trims whitespace from individual broker addresses', () => {
    const original = process.env['KAFKA_BROKERS'];
    try {
      process.env['KAFKA_BROKERS'] = ' localhost:4511 , localhost:4512 ';
  ...
  // Test 18 — msk-env.sh format is compatible with shell sourcing
  // The injection script or Makefile sources msk-env.sh, which sets KAFKA_BROKERS.
  // This test validates the expected format: export KAFKA_BROKERS="localhost:port"
  it('recognizes msk-env.sh format (export KAFKA_BROKERS="host:port")', () => {
    const mskEnvContent = 'export KAFKA_BROKERS="localhost:4511"\n';
    // Simulate shell sourcing: extract the value
    const match = mskEnvContent.match(/KAFKA_BROKERS\s*=\s*["']?([^"'\s]+)/);
    assert.ok(match, 'msk-env.sh must contain KAFKA_BROKERS=... assignment');
    const brokerAddress = match[1];
    assert.ok(
      brokerAddress.includes(':'),
      `Broker address must be host:port format. Got: "${brokerAddress}"`,
    );
    const [host, portStr] = brokerAddress.split(':');
    assert.ok(host.length > 0, 'Host must not be empty');
    const port = parseInt(portStr, 10);
    assert.ok(port > 0 && port <= 65535, `Port must be valid (1-65535). Got: ${port}`);
  });
});
