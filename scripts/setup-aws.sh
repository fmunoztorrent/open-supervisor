...
validate_aws_creds() {
  ...
  echo ""
  echo -e "  ${GREEN}# Opcion 1: IAM user (access key + secret)${NC}"
  echo -e "    1. Ir a AWS Console -> IAM -> Users -> tu usuario"
  echo -e "    2. Security credentials -> Create access key"
  echo -e "    3. Elegir 'Command Line Interface (CLI)'"
  echo -e "    4. Guardar Access Key ID y Secret Access Key"
  echo -e "    5. Ejecutar: aws configure"
  echo ""
  echo -e "  ${GREEN}# Opcion 2: AWS Identity Center / SSO${NC}"
  echo -e "    1. Ir a AWS Console -> IAM Identity Center"
  echo -e "    2. Anotar la URL del portal (ej: d-xxxxxxxx.awsapps.com/start)"
  echo -e "    3. Ejecutar: aws sso configure"
  echo -e "    4. Ejecutar: aws sso login"
  echo ""
  echo -e "  ${GREEN}# Opcion 3: credenciales temporales de la consola${NC}"
  echo -e "    1. Ir a AWS Console y hacer login"
  echo -e "    2. Ejecutar: aws login    (requiere AWS CLI v2.17+)"
  echo ""
  echo -e "  ${GREEN}# Opcion 4: variables de entorno${NC}"
  echo -e "    export AWS_ACCESS_KEY_ID=AKIA..."
